    ...
    <div className="flex min-h-screen flex-col bg-background selection:bg-primary/20">
      ...
      <main className="flex-1 container mx-auto px-4 py-20 flex flex-col gap-12">
        <header className="flex flex-col items-center text-center gap-4 max-w-2xl mx-auto">
          ...
          <h1 className="text-4xl md:text-5xl font-black tracking-tight">Browse by Category</h1>
          <p className="text-muted-foreground text-lg leading-relaxed">
            Discover a wide variety of our premium selection by navigating directly to your desired category segment.
          </p>
        </header>

        {!categories && !error ? (
           <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 animate-pulse">
              {[...Array(6)].map((_, i) => <div key={i} className="h-40 rounded-3xl bg-muted/60" />)}
           </div>
        ) : (
                ...
                <FadeIn key={c._id} delay={idx * 0.1}>
                  ...
                  >
                     <div className="flex items-center justify-between">
                        <h2 className="text-2xl font-bold tracking-tight group-hover:text-primary transition-colors">{c.name}</h2>
                     ...
                     <p className="text-sm text-muted-foreground leading-relaxed line-clamp-2 pr-6">
                        {c.description || "Discover premium quality items meticulously crafted and rigorously tested within this category."}
                     </p>
                     
                     {/* Products count indicator below */}
                     <div className="mt-auto pt-6 flex items-center gap-2">
                        {c.products && c.products.length > 0 ? (
                          <span className="text-xs font-bold uppercase tracking-widest text-emerald-600 bg-emerald-500/10 px-3 py-1 rounded-full">{c.products.length} Products</span>
                        ) : (
                          <span className="text-xs font-bold uppercase tracking-widest text-muted-foreground bg-muted px-3 py-1 rounded-full border border-dashed text-center">Empty</span>
                        )}
                     </div>
                  </Link>
                </FadeIn>
