  ...
  return (
    <form onSubmit={handleSubmit} className="flex flex-col gap-8">
      ...
      <Button
        ...
        className="h-16 rounded-2xl text-xl font-black w-full shadow-2xl shadow-primary/20"
      >
        {loading
          ? "Processing Payment..."
...
const CheckoutPage = () => {
  ...
  const handleOrderSuccess = async (paymentId?: string) => {
    try {
      ...
      toast.error(e.message || "Order finalization failed");
    }
  };

  if (loading)
    return (
      <div className="min-h-screen flex items-center justify-center animate-pulse text-xs font-black uppercase tracking-widest text-muted-foreground">
        Authenticating Checkout Session...
      </div>
    );
  if (!user) return null;
    ...
    <main className="flex-1 py-12 px-4 md:px-8 container mx-auto flex flex-col gap-12 max-w-7xl pb-40">
      <div className="flex flex-col gap-2">
        ...
        <p className="text-xs font-black text-muted-foreground uppercase tracking-widest flex items-center gap-2">
          <ShieldCheck size={14} className="text-emerald-500" /> AES-256 Grade
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 lg:items-start">
        ...
        <div className="lg:col-span-7 flex flex-col gap-10">
          ...
          <section className="flex flex-col gap-6 p-8 rounded-[40px] border bg-card/30">
            <h3 className="text-xl font-black tracking-tight flex items-center gap-3">
              <MapPin size={22} className="text-primary" /> Shipping Destination
            </h3>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              {user.addresses?.map((addr: any, idx: number) => (
                ...
                >
                  ...
                  <div className="flex flex-col gap-1">
                    ...
                    <span className="text-[10px] font-bold text-muted-foreground flex items-center gap-1.5 mt-2">
              ...
              <button
                ...
                className="p-6 rounded-3xl border-2 border-dashed flex flex-col items-center justify-center gap-2 opacity-50 hover:opacity-100 transition-opacity"
              >
                <MapPin size={24} />
              ...
              {method === "COD" ? (
                <div className="flex flex-col gap-6">
                  <div className="flex gap-4 p-4 bg-amber-500/5 text-amber-700 rounded-2xl text-xs font-medium border border-amber-500/10">
                    ...
                    <p>
                      Please keep exactly Rs. {total.toLocaleString()} ready.
                      Our rider will contact you upon arrival at{" "}
                ...
                <div className="p-10 text-center animate-pulse text-xs font-black uppercase tracking-widest opacity-20">
        ...
        <div className="lg:col-span-5 flex flex-col gap-8 sticky top-28">
          <section className="p-10 rounded-[48px] border bg-card shadow-2xl shadow-primary/10 flex flex-col gap-10 overflow-hidden relative">
              ...
              {items.map((item, idx) => (
                <div key={idx} className="flex items-center gap-4 group">
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="font-black text-sm truncate uppercase tracking-tighter leading-none">
                      {item.product.name}
                    </p>
                    <span className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest">
                      {item.quantity} Unit(s) × Rs. {item.product.price}
                    </span>
                  </div>
            ...
            <div className="flex flex-col gap-4 pt-8 border-t border-dashed">
              <div className="flex justify-between items-center text-sm">
                <span className="font-bold text-muted-foreground uppercase tracking-widest italic opacity-50">
                  Subtotal Ledger
                </span>
              ...
              <div className="flex justify-between items-end">
                <span className="text-sm font-black text-primary uppercase tracking-[4px]">
                  Net Total
                </span>
            ...
            <div className="p-6 bg-emerald-500/10 rounded-3xl border border-emerald-500/20 flex items-center gap-4">
              ...
              <div className="flex flex-col">
                ...
                <span className="text-[9px] font-bold text-emerald-600/60 leading-tight">
                  Your order is protected under our high-tier consumer
          ...
          <button
            onClick={() => router.back()}
            className="flex items-center justify-center gap-2 text-xs font-black uppercase tracking-widest text-muted-foreground hover:text-foreground transition-colors"
          >
