...
const CreditSaleInner = () => {
  ...
  }, [localSearch, prevSearch, fetchProducts, stockFilter]);
  ...
  const totalCreditAmount = appendedProducts.reduce(
  ...
  const handleAppendItem = (e: React.FormEvent) => {
    ...
    if (price <= 0) {
      alert("Credit price must be greater than zero");
      return;
    }

    const existingQtyInList = appendedProducts.reduce((sum, item, idx) => {
      if (editingIndex !== null && idx === editingIndex) return sum;
      return item.productId === selectedProduct._id ? sum + item.quantity : sum;
    }, 0);
    const totalNeeded = existingQtyInList + qty;

    if (totalNeeded > selectedProduct.stock) {
      alert(
        `Insufficient stock.\n\nAvailable Stock: ${selectedProduct.stock}\nAlready Appended: ${existingQtyInList}\nAttempted to Add: ${qty}`
      );
      return;
    }
  ...
  const handleDeleteItem = (index: number) => {
    ...
    const next = appendedProducts.filter((_, idx) => idx !== index);
    setAppendedProducts(next);
  ...
  const handleSaveCreditSale = async () => {
    ...
    if (createdCreditSale) {
      const date = createdCreditSale.createdAt
        ? new Date(createdCreditSale.createdAt).toLocaleString("en-PK", {
            day: "2-digit",
            month: "2-digit",
            year: "numeric",
            hour: "2-digit",
      ...
      setLastBillData({
        ...
        type: "Credit",
        totalAmount: createdCreditSale.totalAmount,
        sellerName: createdCreditSale.createdBy?.name || "",
      });
  ...
  return (
    <div className="flex flex-col gap-8">
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-2xl font-black tracking-tight">Credit Sale</h1>
          <p className="text-sm text-muted-foreground mt-0.5">
            Save products provided on credit, deduct stock, and generate a credit receipt
          </p>
        </div>
      </div>
          ...
          <div className="border rounded-2xl bg-card p-6 flex flex-col gap-4 shadow-sm">
            ...
            <div className="flex flex-col gap-2 max-h-[360px] overflow-y-auto pr-2">
              {productsLoading && products.length === 0 ? (
                ...
                products.map((p) => {
                  const quantityInList = appendedProducts.reduce(
                    (sum, item) => item.productId === p._id ? sum + item.quantity : sum,
                    0
                  );
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-xs text-muted-foreground truncate mt-1">
                          ...
                          {quantityInList > 0 && (
                            <span className="text-[10px] bg-primary/10 text-primary px-1.5 py-0.5 rounded font-bold ml-2">
                              Added: {quantityInList}
                            </span>
                          )}
                        </p>
                      </div>
          ...
          >
            <div className="flex justify-between items-center">
              <h2 className="font-bold text-base flex items-center gap-2">
                <span>{editingIndex !== null ? "Edit Credit Item" : "Credit Item Details"}</span>
                {editingIndex !== null && (
            ...
            <label className="flex flex-col gap-1.5">
              ...
              <textarea
                ...
                placeholder="Credit sale details..."
              />
            </label>
            ...
            {appendedProducts.length === 0 ? (
              ...
              <div className="flex flex-col gap-4">
                ...
                >
                  ...
                  {saving ? "Saving Credit Sale..." : "Save Credit Sale & Generate Receipt"}
                </Button>
              </div>
            )}
          </div>
...
const AdminCreditSalePage = () => (
  <CreditSaleProvider>
    <CreditSaleInner />
  </CreditSaleProvider>
);

export default AdminCreditSalePage;
